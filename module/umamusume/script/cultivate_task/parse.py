...
def parse_umamusume_basic_ability_value(ctx: UmamusumeContext, img):
    # Each stat is read with full detection (ocr_line / det=True): PaddleOCR first
    # locates the digit box inside the crop and reads only that. A det=False batch
    # was tried for speed but, lacking the detection step, it read the whole crop
    # (border + anti-aliasing) as one line and hallucinated huge garbage numbers,
    # so we keep the robust per-stat read here.
    ...
    sub_img_intelligence = cv2.copyMakeBorder(sub_img_intelligence, 20, 20, 20, 20, cv2.BORDER_CONSTANT, None, (255, 255, 255))
    intelligence_text = ocr_line(sub_img_intelligence)

    sub_img_skill = img[855:902, 602:690]
    sub_img_skill = cv2.copyMakeBorder(sub_img_skill, 20, 20, 20, 20, cv2.BORDER_CONSTANT, None, (255, 255, 255))
